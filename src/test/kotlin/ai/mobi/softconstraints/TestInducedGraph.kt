...
class TestInducedGraph {
    ...
    @Test
    fun `test lowerNeighbors on simple hypergraph`() {
        /* Generated by ChatGPT */
        val hypergraph = Hypergraph<String, String>()

        // Define a simple variable list (elimination order)
        val vars = listOf("u", "v", "w", "y", "a1", "a2", "e1", "e2", "o1")

        // Add nodes (variables)
        vars.forEach { hypergraph.addNode(it) }

        // Add hyperedges (constraints)
        hypergraph.addHyperedge("f_a1", listOf("a1", "y", "w"))
        hypergraph.addHyperedge("f_a2", listOf("a2", "v", "u"))
        hypergraph.addHyperedge("f_e1", listOf("e1", "y", "u"))
        hypergraph.addHyperedge("f_e2", listOf("e2", "u"))
        hypergraph.addHyperedge("f_o1", listOf("o1", "w", "v"))

        // Expected lower neighbors for each variable
        assertEquals(setOf(), hypergraph.lowerNeighbors("u", vars))    // No neighbors before "u"
        assertEquals(setOf("u"), hypergraph.lowerNeighbors("v", vars))  // "u" is before "v"
        assertEquals(setOf("v"), hypergraph.lowerNeighbors("w", vars))  // "v" and "u" are before "w"
        assertEquals(setOf("w", "u"), hypergraph.lowerNeighbors("y", vars)) // "w, v, u" are before "y"
        assertEquals(setOf("y", "w"), hypergraph.lowerNeighbors("a1", vars)) // "y, w, v, u" are before "a1"
        assertEquals(setOf("v", "u"), hypergraph.lowerNeighbors("a2", vars)) // "v, u" are before "a2"
        assertEquals(setOf("y", "u"), hypergraph.lowerNeighbors("e1", vars)) // "y, u" are before "e1"
        assertEquals(setOf("u"), hypergraph.lowerNeighbors("e2", vars)) // "u" is before "e2"
        assertEquals(setOf("w", "v"), hypergraph.lowerNeighbors("o1", vars)) // "w, v" are before "o1"
    }
}